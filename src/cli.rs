...
fn handle_config_update() {
    ...
    // Generate new config preserving user values
    let updated = generate_config_template(&existing);
...
/// Generate a config file template with the given values
...
"#,
        theme = config.theme,
        use_bg = config.use_theme_background,
        limit = config.context_limit,
        bind = config.bind_addr,
        log_dir = config.log_dir.display(),
        storage = config.features.storage,
        thinking = config.features.thinking_panel,
        stats = config.features.stats,
        ctx_warn = config.augmentation.context_warning,
        thresholds = config.augmentation.context_warning_thresholds,
    )
}
